...
def run_job():
    ...
    def feq(t,y):
        ...
        phi1 =0
        phi2 =0
        omega11=omega1*np.sqrt(1+phigen1(t,phase_arr1))
        omega22=omega2*np.sqrt(1+phigen2(t,phase_arr2))
        derivs=[0.5*1j*omega11*np.exp(1j*(delta1*t+phi1))*b,
                0.5*1j*omega11*np.exp(-1j*(delta1*t+phi1))*a+0.5*1j*omega22*np.exp(1j*(delta2*t+phi2))*c,
                0.5*1j*omega22*np.exp(-1j*(delta2*t+phi2))*b]
...
for i in range(nl):
    ...
    sig1=np.sqrt((lwset*fmax))
    sig2=np.sqrt((lwset*fmax))


    yqsarr.append(0.25*(timenum/2)**2*(np.pi)**2*(sig1**2+sig2**2))
