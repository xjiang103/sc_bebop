...
def phigen(t,phase_arr):
    ...
    return phitemp
def s_nu(f,lw):
    #return (1*2500*np.exp(-(f-lw)**2/(2*120000**2)))/f**2
    if (f<lw):
        return 7000/f**2
    else:
        return 0
...
#    print(res[1])
    ...
    if ((i+1)%avenum==0):
        y4_fmax.append(res[0])
        y4_stdp.append(res[1])
        y4_stdn.append(res[2])
        x4_fmax.append(lwset/(omega_0/(2*math.pi)))
        print(lwset/(omega_0/(2*math.pi)))
        print(res[0])
    eprev=res[0]
