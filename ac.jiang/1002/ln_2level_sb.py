...
def phigen(t,phase_arr):
    ...
    return phitemp
def s_nu(f,lw):
    #return (1*500*(1+1*300/f)-350+1*0.5*(150-500)*(erf((f-40000)/10000)-1)+bpscale*2500*np.exp(-(f-lw)**2/(2*12000**2)))/f**2
    return bpscale*2500*np.exp(-(f-lw)**2/(2*12000**2))/f
...
#    lwset=swparr[i][0]
    print(i)
    tpi=1*tpi0
    lwset=swprange*fmax/nl*i+40000+12000*5
    for k in range(nf):
        pikdf_arr[k] = 2*np.pi*(k+1)*df
#    print(")*(&*")
