...
tpnum=tpi_num
filestr="fmaxswp_0908_"+str(tpnum)+".txt"
f=open(filestr,"w")
units=1e6

#rabi parameters
#rabi parameters
omega1=2*np.pi*(1000)*(units)
omega2=2*np.pi*(1000)*(units)
delta1=2*np.pi*(1000000)*(units)
...
def run_job():
    ...
    sol = solve_ivp(feq,t,y0,rtol=1e-8,atol=3e-8)
...
for i in range(nl):
    ...
    print("numerics:"+str(res[0]))
    y1.append(res[0])
    sp.append(res[1])
    sn.append(res[2])
    sig1=np.sqrt((1/np.pi)*2*lwset1*nf*df)/(omega_0/(2*np.pi))
    sig2=np.sqrt((1/np.pi)*2*lwset2*nf*df)/(omega_0/(2*np.pi))
    ...
    print("quasi-static theory:"+str(quas))
    quasarr.append(quas)
    sigarr.append(sig)
    x1.append(lwset1)
