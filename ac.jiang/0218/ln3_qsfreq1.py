...
for i in range(nl):
    ...
    sig2=np.sqrt((lwset/4*fmax/np.pi)/(omegar/(2*np.pi*units))**1)

    sig=np.sqrt((1-alpha)**2*sig1**2+(1+alpha)**2*sig2**2)
    yqsarr.append(4*sig**2+0.75*0.25*timenum**2*(np.pi)**2*sig**4)
