...
def run_job():
    ...
    #print(yf)
    return [(abs(yf[0]))**2,(abs(yf[1]))**2,cmath.phase(yf[0]),cmath.phase(yf[1]),(abs(yf[0]/np.sqrt(2)-1j*yf[1]/np.sqrt(2)))**2]
...
##        res.append(run_job())
    ...
    return [meanr0,stdp,stdn,meanr1,meanp0,meanp1,meanf]
...
#    print(res[1])
    ...
    y2_fmax.append(res[3])
    p1_fmax.append(res[4])
    p2_fmax.append(res[5])
    f_fmax.append(1-res[6])
...
for i in range(int(nl/avenum)):
    f.write(str(x4_fmax[i])+" "+str(y4_fmax[i])+" "+str(y4_stdn[i])+" "+str(y4_stdp[i])+" "+str(y2_fmax[i])+" "+str(p1_fmax[i])+" "+str(p2_fmax[i])+" "+str(f_fmax[i])+"\n")
